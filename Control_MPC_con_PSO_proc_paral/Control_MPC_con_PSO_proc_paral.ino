...
double x0 = 25;                                           // Posición angular inicial (grados)
...
#define NP 50    // Numero de particulas
double de_min = 0.00000001; // Cota de desviacion estandar relativa para criterio de paro
#define c1 2.0   // Coeficiente de aprendizaje cognitivo
#define c2 2.0  // Coeficiente de aprendizaje social
double alpha_max = 1;
double alpha_min = 0.1;
double iter_max = 500; // Número maximo de iteraciones;
...
double Xbest[NP][n];   // Matriz de "mejor" individual
double gbest[n];       // "Mejor" global
double V[NP][n] = {0}; // Matriz de velocidades
double iter = 0; // iteraciones
double Fss[NP]; // Vector de evaluacion de función de costo
double prom;
double de;
...
void inicializacion()
{
    for (int i = 0; i < NP; i++)
    {
        for (int j = 0; j < n; j++)
        {
            ...
            al = (rand() % (10000 + 1) / 10000.0);
            V[i][j] = xmin[j] + (xmax[j] - xmin[j]) * al;
        }
    }
    memcpy(Xbest, X, NP * n * sizeof(double));
}
...
int regla_deb(double x1[n], double x2[n])
{
    int ganador;
...
void primer_best()
{
    memcpy(gbest, Xbest[0], n * sizeof(double));
    double xt[n];
    int ganador;

    for (int i = 1; i < NP; i++)
    {
        memcpy(xt, X[i], n * sizeof(double));
        ganador = regla_deb(gbest, xt);
        if (ganador == 2)
        {
            memcpy(gbest, xt, n * sizeof(double));
        }
    }
}

void actualizar_best()
{
    double x1[n], x2[n];
    int ganador;
    for (int i = 0; i < NP; i++)
    {
        memcpy(x1, Xbest[i], n * sizeof(double));
        memcpy(x2, X[i], n * sizeof(double));
        ganador = regla_deb(x1, x2);
        if (ganador == 2)
        {
            memcpy(Xbest[i], x2, n * sizeof(double));
            ganador = regla_deb(gbest, x2);
            if (ganador == 2)
            {
                memcpy(gbest, x2, n * sizeof(double));
            }
        }
    }
}
...
{
    double v;
    double alpha = alpha_max - (alpha_max - alpha_min) * (iter / iter_max);
    double r1, r2;
    for (int i = 0; i < NP; i++)
    {
        for (int j = 0; j < n; j++)
        {
            r1 = (rand() % (10000 + 1) / 10000.0);
            r2 = (rand() % (10000 + 1) / 10000.0);
            v = alpha * V[i][j] + c1 * r1 * (Xbest[i][j] - X[i][j]) + c2 * r2 * (gbest[j] - X[i][j]);
            V[i][j] = v;
            X[i][j] = X[i][j] + v;
        }
    }
...
void fitness(){
    double xt[n];
    for(int i=0;i<NP;i++){
        memcpy(xt,Xbest[i],n*sizeof(double));
        Fss[i] = funcion_costo(xt);
    }    
}
...
void optimizador()
{
    ...
    for (iter = 0; iter < iter_max; iter++)
    {
        ...
        //Serial.print(de,10); Serial.print(" | "); Serial.println(iter);
        if(de<de_min){
            break;
        }
    }
    memcpy(U, gbest, n * sizeof(double));
}
